...
with tf.name_scope("sftmax") as scope:
    ...
    s_y = tf.nn.softmax(y_conv)
    # y_conv1 = tf.nn.relu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
    # y_conv1_squre = (y_conv1 * y_conv1)
    # y_conv1_squre_sum = tf.reduce_sum(y_conv1_squre, [1], keep_dims=True)
    # y_ss = y_conv1_squre / y_conv1_squre_sum
...
with tf.name_scope("xent") as scope:
    ...
    logloss2 = -tf.reduce_mean(tf.reduce_sum(y_*tf.log(y_conv), 1))
    logloss3 = -tf.reduce_mean(tf.reduce_sum(y_*tf.log(s_y), 1))
...
def total_eval():
    test_logloss = sess.run(logloss3, feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
    print ("--------------------------------")
    print ("Total logloss of testing data : %s" % test_logloss)
    ...
    for i in range(5):
        ...
        eval_feed = {x: mnist.train.images[start:end], y_: mnist.train.labels[start:end], keep_prob: 1.0}
        print ("test accuracy %g" % accuracy.eval(feed_dict=eval_feed))
        train_logloss = sess.run(logloss3, feed_dict=eval_feed)
...
restore_path = "/Users/will/HelloPycharm/tutorial/export/mnist_conv_bn_panalty_6000/model.ckpt"
...
# import tutorial.mnist_test_data
# a = mnist_test_data.mnist_test
# for i in range(5):
#     start = i * 10000
#     end = (i + 1) * 10000
#     eval_feed = {x: a[start:end], keep_prob: 1.0}
#     rrr = sess.run(y_conv, feed_dict=eval_feed)
#     np.savetxt("/tmp/out%s.csv" % i, rrr, delimiter=",")
#     print ("%s saved" % i)
