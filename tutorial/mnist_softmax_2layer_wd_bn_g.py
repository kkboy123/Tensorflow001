...
def weight_variable_with_decay(shape, name='None', stddev=0.05, wd=None):
...
with tf.name_scope("conv1") as scope:
    ...
    W_conv1 = weight_variable_with_decay([5, 5, 1, c1_input], name='W_conv1', stddev=0.05)
...
with tf.name_scope("conv2") as scope:
    ...
    W_conv2 = weight_variable_with_decay([5, 5, c1_input, c2_input], name='W_conv2', stddev=0.05)
...
''' Fully (densely) connected Layer fc1'''
with tf.name_scope("fc1") as scope:
    # W_fc1 = weight_variable([7 * 7 * c2_input, fc1_input], name='W_fc1')
    W_fc1 = weight_variable_with_decay([7 * 7 * c2_input, fc1_input], name='W_fc1', stddev=0.05, wd=0.004)
...
''' dropout for reducing over-fitting dropout1'''
...
with tf.name_scope("fc2") as scope:
    W_fc2 = weight_variable_with_decay([fc1_input, fc2_input], name='W_fc2', stddev=0.05, wd=0.004)
...
with tf.name_scope("softmax") as scope:
    W_fc3 = weight_variable_with_decay([fc2_input, output_class], name='W_fc3', stddev=0.05)
...
with tf.name_scope("xent") as scope:
    ...
    # logloss = -tf.reduce_sum(y_*tf.log(tf.clip_by_value(y_conv, 1e-25, 1.0))\
    #  + (1 - y_) * tf.log(tf.clip_by_value(1 - y_conv, 1e-25, 1.0)))
    logloss = -tf.reduce_mean(y_*tf.log(tf.clip_by_value(y_conv, 1e-25, 1.0))\
     + (1 - y_) * tf.log(tf.clip_by_value(1 - y_conv, 1e-25, 1.0)))
    ...
    tf.add_to_collection('losses', logloss)
...
for i in range(2501):
    ...
    if i % 100 == 0:
        ...
        loss = result[1]
        loss2 = result[2]
        print("Accuracy at step %s: %s; loss : %s; loss2 : %s" % (i, acc, loss, loss2))
        if np.isnan(loss):
    ...
    if i % 500 == 0 and i > 0:
        total_eval()
    train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.4, keep_prob2: 0.4})
