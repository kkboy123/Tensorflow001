...
with tf.name_scope("fc1") as scope:
    ...
    h_pool2_flat = tf.reshape(h_norm2, [-1, 7*7*64])
...
with tf.name_scope("train") as scope:
    train_step = tf.train.AdamOptimizer(1e-7).minimize(logloss)
...
# writer = tf.train.SummaryWriter("/tmp/mnist_conv_lrn_logs", sess.graph_def)
...
saver.restore(sess, "/Users/will/HelloPycharm/tutorial/export/mnist_conv_lrn_6000/model.ckpt")
# for i in range(6001):
...
#             print("W_conv1 : %s" % result[0])
#             print("W_conv2 : %s" % result[1])
#             print("W_fc1 : %s" % result[2])
#             break
#     train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5})

result = sess.run(logloss2, feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
print ("Total logloss of testing data : %s" % result)
