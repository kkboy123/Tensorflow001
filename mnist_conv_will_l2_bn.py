...
def layer(_input, weight_shape, bias_shape, name="default_name", is_output=False):
    ...
    dot = tf.matmul(_input, w)
    '''batch normalization'''
    mean, var = tf.nn.moments(dot, [0])
    w_norm = (dot - mean) / tf.sqrt(var + epsilon)
    scale_init = tf.constant_initializer(value=1)
    beta_init = tf.constant_initializer(value=0)
    scale = tf.get_variable(name + "_scale", bias_shape, initializer=scale_init)
    beta = tf.get_variable(name + "_beta", bias_shape, initializer=beta_init)
    return tf.nn.relu(scale * w_norm + beta)
...
def conv2d(_input, weight_shape, bias_shape, name="default_name"):
    ...
    # bias_init = tf.constant_initializer(value=0)
    # b = tf.get_variable(name + "_b", bias_shape, initializer=bias_init)
    conv = tf.nn.conv2d(_input, w, strides=[1, 1, 1, 1], padding='SAME')
    '''batch normalization'''
    mean, var = tf.nn.moments(conv, [0, 1, 2])
    w_norm = (conv - mean) / tf.sqrt(var + epsilon)
    scale_init = tf.constant_initializer(value=1)
    beta_init = tf.constant_initializer(value=0)
    scale = tf.get_variable(name + "_scale", bias_shape, initializer=scale_init)
    beta = tf.get_variable(name + "_beta", bias_shape, initializer=beta_init)
...
def inference(_x, _keep_prob):
    ...
    with tf.variable_scope("conv_1"):
        conv_1 = conv2d(_x, [5, 5, 1, c1_input], [c1_input], name="conv1")
        pool_1 = max_pool(conv_1)

    with tf.variable_scope("conv_2"):
        conv_2 = conv2d(pool_1, [5, 5, c1_input, c2_input], [c2_input], name="conv2")
        pool_2 = max_pool(conv_2)

    with tf.variable_scope("fc"):
        pool_2_flat = tf.reshape(pool_2, [-1, 7 * 7 * c2_input])
        fc_1 = layer(pool_2_flat, [7 * 7 * c2_input, fc1_input], [fc1_input], name="fc1")
        '''apply dropout'''
        fc_1_drop = tf.nn.dropout(fc_1, _keep_prob, name="dropout1")

    with tf.variable_scope("output"):
        _output = layer(fc_1_drop, [fc1_input, output_class], [output_class], name="fc2", is_output=True)
...
def total_eval(validation_size=50):
    total_loss = 0
    print("---------------")
    for j in range(5):
        start = j * 10000
        end = start + min(validation_size,10000)
...
if __name__ == '__main__':
    ...
    load_model = True
    if load_model:
        restore_path = "/Users/will/HelloPycharm/tutorial/export/will/l2_bn_0053/model_conv.ckpt"
