...
restore_path = "/Users/will/HelloPycharm/tutorial/export/mlx_0.052/model.ckpt"
...
for i in range(epoch_size):
    ...
    for j in range(int(10000/batch_size)):
        ...
        if j % 100 == 0:
            ...
            my_eval(feed)
            # result = sess.run([accuracy, logloss, logloss2, cost], feed_dict=feed)
            ...
            # print("Accuracy at step %s: %s; loss : %s; logloss: %s; cost: %s" %
            #       (j, acc, loss, loss2, total_cost))
            # if np.isnan(loss):
            #     print("logloss2 : %s" % result[2])
            #     print("y_conv : %s" % result[3])
            #     break
        train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.35})
    total_eval(100)
total_eval(10000)
