...
def total_eval():
    test_logloss = sess.run(logloss2, feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
    print ("--------------------------------")
...
for i in range(5):
    ...
    out_seq.append(DAT)
    # np.savetxt("/tmp/out%s.csv" % i, DAT, delimiter=",", fmt="%s")
    # np.savetxt("/tmp/id.csv", np.array(b[start:end]), delimiter=",",fmt="")
    # print ("%s saved" % i)
